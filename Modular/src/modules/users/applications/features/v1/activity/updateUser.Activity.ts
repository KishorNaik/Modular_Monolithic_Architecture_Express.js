...
} from '@/modules/users/contracts/features/updateUser.Contract';
...
export class UpdateUserController {
	...
	public async updateUserAsync(
		...
		const response = await mediatR.send<DataResponse<UpdateUserRequestDTO>>(
...
class UpdateUserCommandHandler
	implements IRequestHandler<UpdateUserCommand, DataResponse<UpdateUserResponseDTO>>
{
	...
	private async updateAsync(value: UpdateUserCommand): Promise<Result<boolean, HttpException>> {
		try {
			...
			if (result.affected === 0)
				return new Err(new HttpException(StatusCodes.BAD_REQUEST, 'User not found'));
