...
export class CreateUserController {
  ...
  public async addUserAsync(@Body() createUserRequestDTO: CreateUserRequestDTO, @Res() res: Response) {
    const response = await mediatR.send<DataResponse<CreateUserResponseDTO>>(new CreateUserCommand(createUserRequestDTO));
...
class CreateUserCommandHandler implements IRequestHandler<CreateUserCommand, DataResponse<CreateUserResponseDTO>> {
  ...
  private async isOrgExistsAsync(id: number): Promise<Result<boolean, HttpException>> {
    try {
      ...
      // check if org exists
      const getOrgExistsIntegrationServiceResult: DataResponse<GetOrgByIdResponseDTO> = await mediatR.send<DataResponse<GetOrgByIdResponseDTO>>(
  ...
  private async addAsync(userEntity: UserEntity): Promise<Result<number, HttpException>> {
    try {
      if (!userEntity) return new Err(new HttpException(StatusCodes.BAD_REQUEST, 'userEntity is null'));

      const result = await this.appDataSource.manager.createQueryBuilder().insert().into(UserEntity).values(userEntity).execute();

      if (!result.identifiers[0].id) return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'userEntity is null'));
  ...
  public async handle(value: CreateUserCommand): Promise<DataResponse<CreateUserResponseDTO>> {
    ...
    if (isOrgExistsResult.isErr()) return CommandException.commandError(isOrgExistsResult.error.message, isOrgExistsResult.error.status);

    // Generate Hash Password.
    const hashedPassword = await this.hashPasswordService.hashPasswordAsync(value.createUserRequestDTO.password);
    if (hashedPassword.isErr()) return CommandException.commandError(hashedPassword.error.message, hashedPassword.error.status);
    ...
    if (addUserResult.isErr()) return CommandException.commandError(addUserResult.error.message, addUserResult.error.status);
