...
} from '@/modules/users/contracts/features/refreshToken.Contracts';
...
export class RefreshTokenController {
	...
	public async refreshTokenAsync(
		...
		const response = await mediatR.send<DataResponse<RefreshTokenResponseDTO>>(
			new RefreshTokenCommand(refreshTokenRequestDTO)
		);
...
class RefreshTokenCommandHandler
	implements IRequestHandler<RefreshTokenCommand, DataResponse<RefreshTokenResponseDTO>>
{
	...
	): Promise<Result<string, HttpException>> {
		const getClaimsFromRefreshTokenPromise: Promise<IClaims> =
			this.jwtTokenService.getClaimsFromRefreshTokenAsync(
				refreshTokenRequestDTO.refreshToken
			);
		const getClaimsFromAccessTokenPromise: Promise<IClaims> =
			this.jwtTokenService.getClaimsFromAccessTokenAsync(refreshTokenRequestDTO.accessToken);

		const [getClaimsFromRefreshTokenResult, getClaimsFromAccessTokenResult] = await Promise.all(
			[getClaimsFromRefreshTokenPromise, getClaimsFromAccessTokenPromise]
		);

		if (!getClaimsFromAccessTokenResult)
			return new Err(new HttpException(StatusCodes.UNAUTHORIZED, 'Invalid Access Token'));

		if (!getClaimsFromRefreshTokenResult)
			return new Err(new HttpException(StatusCodes.UNAUTHORIZED, 'Invalid Refresh Token'));
	...
	public async handle(
		...
		try {
			...
			if (getUserIdResult.isErr())
				return CommandException.commandError(
					getUserIdResult.error.message,
					getUserIdResult.error.status
				);

			// Get User By Id
			const getUserByIdResult = await this.userSharedRepository.getUserByIdAsync(
				parseInt(getUserIdResult.value)
			);

			if (getUserByIdResult.isErr())
				return CommandException.commandError('Invalid User', StatusCodes.UNAUTHORIZED);

			const userEntity: UserEntity = getUserByIdResult.value;

			// Verify Refresh Token
			const verifyRefreshTokenResult = this.verifyRefreshToken(
				value.request.refreshToken,
				userEntity.refreshToken
			);
			if (verifyRefreshTokenResult.isErr())
				return CommandException.commandError(
					verifyRefreshTokenResult.error.message,
					verifyRefreshTokenResult.error.status
				);
