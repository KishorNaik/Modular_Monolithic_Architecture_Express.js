...
export class RefreshTokenController{
    ...
    public async refreshTokenAsync(@Body() refreshTokenRequestDTO:RefreshTokenRequestDTO,@Res() res: Response) {
        ...
        let response=await mediatR.send<DataResponse<RefreshTokenResponseDTO>>(new RefreshTokenCommand(refreshTokenRequestDTO));
...
class RefreshTokenCommandHandler implements IRequestHandler<RefreshTokenCommand,DataResponse<RefreshTokenResponseDTO>>{
    ...
    private async validateTokenAsync(refreshTokenRequestDTO:RefreshTokenRequestDTO) : Promise<Result<string,HttpException>>{
        ...
        const getClaimsFromRefreshTokenPromise:Promise<IClaims>=this.jwtTokenService.getClaimsFromRefreshTokenAsync(refreshTokenRequestDTO.refreshToken);
        ...
        const [getClaimsFromRefreshTokenResult,getClaimsFromAccessTokenResult]=await Promise.all([getClaimsFromRefreshTokenPromise,getClaimsFromAccessTokenPromise]); 
    ...
    private response(tokensValue:tokenTuples): DataResponse<RefreshTokenResponseDTO>{
        ...
        let refreshTokenResponseDTO:RefreshTokenResponseDTO=new RefreshTokenResponseDTO();
    ...
    public async handle(value: RefreshTokenCommand): Promise<DataResponse<RefreshTokenResponseDTO>> {
        try
        {
            ...
            if(getUserIdResult.isErr())
                return CommandException.commandError(getUserIdResult.error.message,getUserIdResult.error.status);
