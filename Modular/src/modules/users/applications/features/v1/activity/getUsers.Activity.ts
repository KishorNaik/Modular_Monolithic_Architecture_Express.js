...
export class GetUsersController{
    ...
    public async getUserByIdAsync(@QueryParams() getUsersFilterRequestDTO:GetUsersFilterRequestDTO,@Res() res: Response) {
...
class GetUserQueryHandler implements IRequestHandler<GetUserQuery, DataResponse<GetUsersFilterResponseDTO[]>> {
    ...
    private fullNameFilter(fullName:string,selectQueryBuilder:SelectQueryBuilder<UserEntity>):Result<SelectQueryBuilder<UserEntity>,HttpException>{
        try
        {
    ...
    private response(users:GetUsersFilterResponseDTO[], pageList:PagedList<UserEntity>):DataResponse<GetUsersFilterResponseDTO[]>{
        ...
        if(users.length===0)
            return DataResponseFactory.Response<GetUsersFilterResponseDTO[]>(false,StatusCodes.NOT_FOUND,users,"Users not found");

        let paginationModel=new PaginationDataResponseModel()
