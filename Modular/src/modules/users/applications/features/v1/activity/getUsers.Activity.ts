...
import {
	GetUsersFilterRequestDTO,
	GetUsersFilterResponseDTO,
} from '@/modules/users/contracts/features/getUsersFilter.Contracts';
...
export class GetUsersController {
	...
	public async getUserAsync(
		@QueryParams() getUsersFilterRequestDTO: GetUsersFilterRequestDTO,
		@Res() res: Response
	) {
		const response = await mediatR.send<DataResponse<GetUsersFilterResponseDTO>>(
			new GetUserQuery(getUsersFilterRequestDTO)
		);
...
class GetUserQueryHandler
	implements IRequestHandler<GetUserQuery, DataResponse<GetUsersFilterResponseDTO[]>>
{
	...
	private fullNameFilter(
		...
		try {
			if (fullName !== undefined) {
				return new Ok(
					selectQueryBuilder.where('fullName like :fullName', {
						fullName: `%${fullName}%`,
					})
				);
			}
	...
	private emailIdFilter(
		...
		try {
			if (emailId !== undefined) {
				return new Ok(
					selectQueryBuilder.where('emailId like :emailId', { emailId: `%${emailId}%` })
				);
			}
	...
	private response(
		users: GetUsersFilterResponseDTO[],
		pageList: PagedList<UserEntity>
	): DataResponse<GetUsersFilterResponseDTO[]> {
		if (users.length === 0)
			return DataResponseFactory.Response<GetUsersFilterResponseDTO[]>(
				false,
	...
	public async handle(value: GetUserQuery): Promise<DataResponse<GetUsersFilterResponseDTO[]>> {
		try {
			if (!value)
				return QueryException.queryError('request is empty', StatusCodes.BAD_REQUEST);
			...
			if (selectQueryBuilderResult.isErr())
				return QueryException.queryError(
					selectQueryBuilderResult.error.message,
					selectQueryBuilderResult.error.status
				);

			let selectQueryBuilder = selectQueryBuilderResult.value;

			// Filter full Name
			selectQueryBuilderResult = this.fullNameFilter(
			...
			// Filter emailId
			selectQueryBuilderResult = this.emailIdFilter(
			...
			const pageList = await PagedList.toPagedListAsync(
				selectQueryBuilder,
