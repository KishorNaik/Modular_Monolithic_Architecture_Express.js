...
import { Delete, HttpCode, JsonController, OnUndefined, Param, Post, Res, UseBefore } from "routing-controllers";
...
import { RemoveOrganizationIntegrationService, RemoveOrganizationRollbackIntegrationService } from "@/modules/organization/applications/features/v1/activity/removeOrganization.Activity";
...
class RemoveUserCommandHandler implements IRequestHandler<RemoveUserCommand, DataResponse<RemoveUserResponseDTO>> {
    ...
    public constructor() {
        this.appDataSource=Container.get(UserDataSource);
    ...
    public async handle(value: RemoveUserCommand): Promise<DataResponse<RemoveUserResponseDTO>> {
        ...
        try
        {
            ...
            if(removeSagaOrchestrator===false)
                return CommandException.commandError("Remove user failed",StatusCodes.INTERNAL_SERVER_ERROR);
...
class RemoveUserSagaOrchestratorHandler implements IRequestHandler<RemoveUserSagaOrchestrator, boolean> {
    ...
    private async removeUserAsync(userId:number, queryRunner:QueryRunner):Promise<Result<boolean,HttpException>>{
        ...
        try
        {
            var result=await this.appDataSource.manager.createQueryBuilder(queryRunner)
                                                    .update(UserEntity)
                                                    .set({
            ...
            if(result.affected==0)
                return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR,"User not found"));
    ...
    public async handle(value: RemoveUserSagaOrchestrator): Promise<boolean> {
        try
        {
            ...
            sagaBuilder.activity("Remove-Activity",async ()=>{
            ...
            if(sagaActivityResult.sagaResult.IsSuccess===false)
                return false;
