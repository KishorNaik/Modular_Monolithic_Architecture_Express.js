...
} from '@/modules/users/contracts/features/removeUser.Contracts';
...
export class RemoveUserController {
	...
	public async removeAsync(@Req() req: Request, @Res() res: Response) {
		...
		const response = await mediatR.send<DataResponse<RemoveUserResponseDTO>>(
			new RemoveUserCommand(request)
		);
...
class RemoveUserCommandHandler
	implements IRequestHandler<RemoveUserCommand, DataResponse<RemoveUserResponseDTO>>
{
	...
	public async handle(value: RemoveUserCommand): Promise<DataResponse<RemoveUserResponseDTO>> {
		...
		try {
			...
			// Get user data by Id
			const userEntityResult = await this.userSharedRepository.getUserByIdAsync(
				value.request.id,
				queryRunner
			);
			if (userEntityResult.isErr())
				return CommandException.commandError('User not found', StatusCodes.NOT_FOUND);
			...
			const removeSagaOrchestrator: boolean = await mediatR.send<boolean>(
				new RemoveUserSagaOrchestrator(userEntity, queryRunner)
			);
			if (removeSagaOrchestrator === false)
...
class RemoveUserSagaResult {
	public constructor(
		removeOrgResult: Result<boolean, HttpException>,
		removeUserResult: Result<boolean, HttpException>,
...
class RemoveUserSagaOrchestratorHandler
	implements IRequestHandler<RemoveUserSagaOrchestrator, boolean>
{
	private readonly appDataSource: DataSource = null;
	...
	private async removeOrgAsync(orgId: number): Promise<Result<boolean, HttpException>> {
		const removeOrgResult = await mediatR.send<DataResponse<RemoveOrganizationResponseDTO>>(
			new RemoveOrganizationIntegrationService(orgId)
		);
		if (removeOrgResult.Success === false)
			return new Err(new HttpException(removeOrgResult.StatusCode, removeOrgResult.Message));

		return new Ok(true);
	}

	private async removeUserAsync(
		userId: number,
		queryRunner: QueryRunner
		...
		try {
			...
			if (result.affected == 0)
				return new Err(
					new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'User not found')
				);
	...
	public async handle(value: RemoveUserSagaOrchestrator): Promise<boolean> {
		try {
			const sagaBuilder: SagaBuilder<RemoveUserSagaResult> = new SagaBuilder(
				'Remove-User-Saga-Orchestrator'
			);
			...
			sagaBuilder
				.activity('Remove-Activity', async () => {
					...
					// Remove User
					const removeUserResult = await this.removeUserAsync(
						value.userEntity.id,
						value.queryRunner
					);
				...
				.compensationActivity(
					'Remove-Activity',
					'Remove-User-RollBack',
					async (sagaResult) => {
						// Revert Remove Organization
						if (sagaResult.Results.removeUserResult.isErr()) {
							await mediatR.send<DataResponse<RemoveOrganizationResponseDTO>>(
								new RemoveOrganizationRollbackIntegrationService(
									value.userEntity.orgId
								)
							);
			...
			const sagaActivityResult = Enumerable.from(sagaBuilder.activityResults).firstOrDefault(
				(x) => x.activityName === 'Remove-Activity'
			);
