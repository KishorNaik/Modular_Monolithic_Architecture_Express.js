...
export class UserLoginController {
  ...
  public async loginAsync(@Body() userLoginRequestDTO: UserLoginRequestDTO, @Res() res: Response) {
    const response = await mediatR.send<DataResponse<UserLoginResponseDTO>>(new UserLoginCommand(userLoginRequestDTO));
...
export class UserLoginCommandHandler implements IRequestHandler<UserLoginCommand, DataResponse<UserLoginResponseDTO>> {
  ...
  public async handle(value: UserLoginCommand): Promise<DataResponse<UserLoginResponseDTO>> {
    try {
      // Check argument
      if (!value) return CommandException.commandError('UserLoginCommand is required', StatusCodes.BAD_REQUEST);
      ...
      if (isValidPasswordResult.isErr()) return CommandException.commandError('Invalid User Name & Password', StatusCodes.FORBIDDEN);
