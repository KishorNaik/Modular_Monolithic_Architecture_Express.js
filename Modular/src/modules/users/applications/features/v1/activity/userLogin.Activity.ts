...
import { JwtTokenResponseDTO, UserLoginRequestDTO, UserLoginResponseDTO, UserResponseDTO } from "@/modules/users/contracts/features/userLogin.Contracts";
...
import { IJwtExtendedService, IJwtService, JwtExtendedService, JwtService,tokenTuples } from "../../../shared/services/jwt.service";
...
export class UserLoginController{
    ...
    public async loginAsync(@Body() userLoginRequestDTO:UserLoginRequestDTO,@Res() res: Response) {

        let response=await mediatR.send<DataResponse<UserLoginResponseDTO>>(new UserLoginCommand(userLoginRequestDTO));
...
export class  UserLoginCommandHandler implements IRequestHandler<UserLoginCommand, DataResponse<UserLoginResponseDTO>> {
    ...
    private response(userEntity:UserEntity,tokenTuples:tokenTuples):DataResponse<UserLoginResponseDTO>{
        let userLoginResponseDTO:UserLoginResponseDTO=new UserLoginResponseDTO();
        ...
        return DataResponseFactory.Response<UserLoginResponseDTO>(true,StatusCodes.OK,userLoginResponseDTO,'User login successfully');
    }
    ...
    public async handle(value: UserLoginCommand): Promise<DataResponse<UserLoginResponseDTO>> {
        try{
            ...
            // Check argument
            if(!value)
                return CommandException.commandError('UserLoginCommand is required',StatusCodes.BAD_REQUEST);

            // Get User By Email Id
            const getUserByEmailIdResult=await this.userSharedRepository.getUserWithPasswordByEmailAsync(value.request.emailId);

            if(getUserByEmailIdResult.isErr())
                return CommandException.commandError(getUserByEmailIdResult.error.message,getUserByEmailIdResult.error.status);

            // Get User Entity
            const userEntity:UserEntity=getUserByEmailIdResult.value;

            // Validate Password
            const isValidPasswordResult=await this.hashPasswordService.comparePasswordAsync(value.request.password,userEntity.password);

            if(isValidPasswordResult.isErr())
                return CommandException.commandError("Invalid User Name & Password",StatusCodes.FORBIDDEN);
