    ...
    public async generateJwtTokenAsync(claims: IClaims): Promise<Result<tokenTuples, HttpException>> {
        try
        {
            ...
            if(!generateJwtTokenResult)
                return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR,'jwt token generation error'));

            if(!generateRefreshTokenResult)
                return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR,'refresh token generation error'));
