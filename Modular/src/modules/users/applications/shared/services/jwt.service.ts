...
export class JwtService implements IJwtService {
  public generateTokenAsync(claims: IClaims): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        const token: string = jwt.sign(claims, SECRET_KEY, { expiresIn: '1h', algorithm: 'HS256' });
        resolve(token);
      } catch (ex) {
        reject(ex);
      }
  ...
  public generateRefreshTokenAsync(claims: IClaims): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        const token: string = jwt.sign(claims, REFRESH_SECRET_KEY, { expiresIn: '7d', algorithm: 'HS256' });
        resolve(token);
      } catch (ex) {
        reject(ex);
      }
...
export class JwtExtendedService implements IJwtExtendedService {
  ...
  public async generateJwtTokenAsync(claims: IClaims): Promise<Result<tokenTuples, HttpException>> {
    try {
      ...
      const [generateJwtTokenResult, generateRefreshTokenResult] = await Promise.all([generateJwtTokenPromise, generateRefreshTokenPromise]);

      if (!generateJwtTokenResult) return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'jwt token generation error'));

      if (!generateRefreshTokenResult) return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'refresh token generation error'));
