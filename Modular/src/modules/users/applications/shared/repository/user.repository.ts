...
export interface IUserSharedRepository {
	...
	): Promise<Result<UserEntity, HttpException>>;
	getUserByIdAsync(
		id: number,
		queryRunner?: QueryRunner
	): Promise<Result<UserEntity, HttpException>>;
	getUserWithPasswordByEmailAsync(
		email: string,
		queryRunner?: QueryRunner
	): Promise<Result<UserEntity, HttpException>>;
...
export class UserSharedRepository implements IUserSharedRepository {
	...
	public async getUserWithPasswordByEmailAsync(
		email: string,
		queryRunner?: QueryRunner
	): Promise<Result<UserEntity, HttpException>> {
		try {
	...
	public async updateRefreshTokenAsync(
		id: number,
		refreshToken: string,
		queryRunner?: QueryRunner
	): Promise<Result<boolean, HttpException>> {
