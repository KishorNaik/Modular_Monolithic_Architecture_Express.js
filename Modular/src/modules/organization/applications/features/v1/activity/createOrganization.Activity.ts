...
} from '@/modules/organization/contracts/features/createOrganization.Contract';
...
export class CreateOrganizationController {
	...
	public async createOrganizationAsync(
		...
		const response = await mediatR.send<DataResponse<CreateOrganizationResponseDTO>>(
			new CreateOrganizationCommand(createOrganizationRequestDTO)
		);
...
class CreateOrganizationCommandHandler
	implements
		IRequestHandler<CreateOrganizationCommand, DataResponse<CreateOrganizationResponseDTO>>
{
	...
	private async addAsync(orgEntity: OrgEntity): Promise<Result<number, HttpException>> {
		try {
			...
			if (!result)
				return new Err(
					new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'result is null')
				);
	...
	private response(id: number): DataResponse<CreateOrganizationResponseDTO> {
		const createOrganizationResponseDTO: CreateOrganizationResponseDTO =
			new CreateOrganizationResponseDTO();
	...
	public async handle(
	...
	): Promise<DataResponse<CreateOrganizationResponseDTO>> {
		// Check argument
		if (!value)
		...
		if (addOrgResult.isErr())
			return CommandException.commandError(
				addOrgResult.error.message,
				addOrgResult.error.status
			);
...
class OrganizationCreatedDomainEventHandler
	implements INotificationHandler<OrganizationCreatedDomainEvent>
{
