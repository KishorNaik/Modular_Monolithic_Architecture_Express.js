...
class RemoveOrganizationCommandHandler implements IRequestHandler<RemoveOrganizationCommand, DataResponse<RemoveOrganizationResponseDTO>> {
  ...
  private async removeAsync(removeOrganizationRequestDTO: RemoveOrganizationRequestDTO): Promise<Result<boolean, HttpException>> {
    try {
      ...
      if (result.affected == 0) return new Err(new HttpException(StatusCodes.INTERNAL_SERVER_ERROR, 'Organization not found'));
  ...
  public async handle(value: RemoveOrganizationCommand): Promise<DataResponse<RemoveOrganizationResponseDTO>> {
    try {
      ...
      if (removeOrgResult.isErr()) return CommandException.commandError(removeOrgResult.error.message, removeOrgResult.error.status);
...
export class RemoveOrganizationIntegrationServiceHandler
  implements IRequestHandler<RemoveOrganizationIntegrationService, DataResponse<RemoveOrganizationResponseDTO>>
{
...
export class RemoveOrganizationRollbackIntegrationServiceHandler
  implements IRequestHandler<RemoveOrganizationRollbackIntegrationService, DataResponse<RemoveOrganizationResponseDTO>>
{
  private readonly appDataSource: DataSource = null;
