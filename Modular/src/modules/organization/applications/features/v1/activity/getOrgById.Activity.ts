...
import {
	IOrgSharedRepository,
	OrgSharedRepository,
} from '../../../shared/repository/org.repository';
...
} from '@/modules/organization/contracts/features/getOrgById.Contracts';
...
export class GetOrgByIdController {
	...
	public async getUserByIdAsync(@Param('id') userId: number, @Res() res: Response) {
		...
		const response = await mediatR.send<DataResponse<GetOrgByIdResponseDTO>>(
			new GetOrgByIdQuery(request)
		);
...
class GetOrgByIdQeryHandler
	implements IRequestHandler<GetOrgByIdQuery, DataResponse<GetOrgByIdResponseDTO>>
{
	...
	private response(
		getOrgByIdResponseDTO: GetOrgByIdResponseDTO
	): DataResponse<GetOrgByIdResponseDTO> {
		return DataResponseFactory.Response<GetOrgByIdResponseDTO>(
			true,
			StatusCodes.OK,
			getOrgByIdResponseDTO,
			'Org found successfully'
		);
	}
	...
	public async handle(value: GetOrgByIdQuery): Promise<DataResponse<GetOrgByIdResponseDTO>> {
		try {
			...
			// get org By Id
			const getOrgByIdResult = await this.orgSharedRepository.getOrgByIdAsync(
				value.request.id
			);

			if (getOrgByIdResult.isErr())
				return QueryException.queryError(
					getOrgByIdResult.error.message,
					getOrgByIdResult.error.status
				);
...
export class GetOrgByIdIntegrationServiceHandler
	implements IRequestHandler<GetOrgByIdIntegrationService, DataResponse<GetOrgByIdResponseDTO>>
{
	public async handle(
		value: GetOrgByIdIntegrationService
