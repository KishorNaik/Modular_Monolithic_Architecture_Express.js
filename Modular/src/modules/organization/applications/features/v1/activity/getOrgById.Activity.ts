...
class GetOrgByIdQeryHandler implements IRequestHandler<GetOrgByIdQuery, DataResponse<GetOrgByIdResponseDTO>> {
  ...
  public async handle(value: GetOrgByIdQuery): Promise<DataResponse<GetOrgByIdResponseDTO>> {
    try {
      ...
      // get org By Id
      const getOrgByIdResult = await this.orgSharedRepository.getOrgByIdAsync(value.request.id);

      if (getOrgByIdResult.isErr()) return QueryException.queryError(getOrgByIdResult.error.message, getOrgByIdResult.error.status);
