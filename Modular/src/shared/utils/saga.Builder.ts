...
class CompensationActivityModel<T> {
...
class SagaActivity<T> implements ISagaActivity<T> {
  compensations: Array<(arg: SagaResult<T>) => Promise<void>> = [];
    ...
    async executeAsync(): Promise<void> {
        for (const activity of this.activities) {
            try {
                ...
                if (!result.isSuccess && activity.compensations.length > 0) {
